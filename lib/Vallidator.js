...
function reviewChain (chain, warning) {
  ...
  chain.slice(0,-1).forEach(function (link) {
    ...
    var str = fill('-',(link.depth+1)*2) + '| ' + commands[link.type] + ' (' + args.join(', ') + ')';
    string.push(str);
  });
...
module.exports = Vallidator = function (usesInitialWidth) {
   ...
   *
   * TODO: make the prieview work as expected if 
   * you use initial width
   *
   */
  ...
  self.checkLink = function (newLink, workingNest, arguments) {
    ...
    self.links.push({
      depth: workingNest ? workingNest.depth : 0,
      type: newLink,
      args: arguments
    });
    self.idx += 1;
    ...
    if(invalid) {
      ...
      var message = [
        ...
        null,
        'A .' + commands[previousLink] + '() cannot be followed by a .' + commands[newLink] + '() command',
        null
      ].join('\n');
