...
module.exports = Vallidator = function (usesInitialWidth) {
  ...
  self.checkOverflow = function (workingNest) {
    if (workingNest.expects < 0) {
      ...
      console.log(fill('-',60))
      console.log('> Grid chain fails at the', getGetOrdinal(self.idx+1), 'link\n');
      console.log(reviewChain(self.links, 'overflowed'));
      console.log('> Column exceeds its parent\'s capacity by', excess);
      console.log(fill('-',60))

      // gridError(failureTypes.overflow, 'bad grid');
      b();
...
function gridError (type, message) {
  try {
    ...
    err.stack.forEach(function (frame, idx) {
      if (frame.getFunctionName()==='chainLink') {
        ...
        console.error(mainErr.getFileName(),mainErr.getLineNumber());
      }
    });

    console.error(fill('-',60))
