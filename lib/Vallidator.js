...
function gridError (idx, type, message) {
  ...
  try {
    ...
    finalMsg.push(fill('-',70));
    finalMsg.push('> '+ type +' caused the', getGetOrdinal(idx), 'link');
    finalMsg.push(message);
    
    // console.error(type);
    err.stack.forEach(function (frame, idx) {
      if (frame.getFunctionName()==='chainLink') {
        var mainErr = err.stack[idx + 1];
        finalMsg.push('> ' + mainErr.getFileName());
        finalMsg.push('> Line: ' + mainErr.getLineNumber());
      }
    });
