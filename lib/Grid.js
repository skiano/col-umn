...
module.exports = Grid = function (vallidator) {
  ...
  self.column = chainFn(LINK_TYPE.COLUMN, function column (width) {
    ...
    if (!isRoot) {
       ...
       * it easy to not wory about siblings
       *
       */
       ...
       * it fills the rest of the row
       */
      width = width || workingNest.expects 
      ...
      console.log('Expect:', workingNest.expects)
      ...
      if (workingNest.expects <= 0) {
         ...
         *
         */
        workingNest = workingNest.parent;

        /*
         * Reset Parent expectation
         * to make room for another row
         *
         */
  ...
  self.nest = chainFn(LINK_TYPE.NEST, function nest (divisions) {
    ...
    if (!workingNest) {
      ...
      /*
       * Create the first working nest
       *
       */
      workingNest = {
        fractional: divisions ? rootColumn.width : false,
        parent: null,
        column: rootColumn,
        expects: rootColumn.width
      }

    } else {

      /*
       * create a deeper nest
       *
       */
      var newColumn = {
        width: workingNest.expects
      };

      workingNest.column.children.push(newColumn);

      var newNest = {
        fractional: divisions ? true : workingNest.fractional,
        parent: workingNest,
        column: newColumn,
        expects: workingNest.expects
      }
