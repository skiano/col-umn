 ...
 *
 */
module.exports = Grid = function (initialWidth, vallidator) {
  ...
  function chainFn (linkType, fn) {
    return function () {
       ...
       */
      vallidator.checkLink(linkType, currentDepth);
      /*
       * Create the link
       */
  ...
  self.column = chainFn(LINK_TYPE.COLUMN, function (width) {
    ...
    if (isRoot) {
       ...
       *
       */
      vallidator.checkOverflow(workingNest, currentDepth);
    }
  });
  ...
  self.nest = chainFn(LINK_TYPE.NEST, function (divisions) {
     ...
     *
     * store if it is fractional so
     * the vallidator can make better mesages 
     *
     */
    var newNest = {
      expects: divisions,
      fractional: workingColumn.fractional,
