...
module.exports = Grid = function (vallidator) {
   ...
   *
   */
  var setOption = chainFn(LINK_TYPE.OPTION, function setOption (option, value) {
  ...
  supportOption = function (methodName, optionName, setter) {
    self[methodName] = function (value) {
      if(setter) {
        return setOption(optionName, setter(workingColumn[optionName]));        
      } else {
  ...
  /*
   * Enable support from prototype
   *
   */
  var supported = self.__supportedOptions__;

  for (method in supported) {
    if (supported.hasOwnProperty(method) && // block prototype chain
         !self.hasOwnProperty(method)) { // ensure name is available
      /*
